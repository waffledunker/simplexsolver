...
#define VMAX  10  //max. number of constraints
  ...
  void Data() {
    ...
    printf(" MAXIMIZE THE PROBLEM(N FOR MIN) (Y/N) ? "); scanf("%c", &R);
    printf("\n NUMBER OF VARIABLES OF THE FUNCTION(AMAC FONKSIYONU DEGISKEN SAYISI) ? "); scanf("%d", &NV);
    printf("\n NUMBER OF CONSTRAINTS(KISIT DENKLEMLERI SAYISI) ? "); scanf("%d", &NC);
    if (R == 'Y' || R=='y')
      R1 = 1.0;
    else
      R1 = -1.0;
    printf("\n INPUT COEFFICIENTS OF THE FUNCTION(FONKSIYON KATSAYILARI):\n");
    for (J = 1; J<=NV; J++) {
      printf("       #%d ? ", J); scanf("%lf", &R2);
      TS[1][J+1] = R2 * R1;
    }
    printf("       Right hand side(0 FOR UNKNOWN VALUE Z) ? "); scanf("%lf", &R2);
    TS[1][1] = R2 * R1;
    for (I = 1; I<=NC; I++) {
      printf("\n CONSTRAINT(KATSAYI) #%d:\n", I);
      for (J = 1; J<=NV; J++) {
        printf("       #%d ? ", J); scanf("%lf", &R2);
        TS[I + 1][J + 1] = -R2;
      }
